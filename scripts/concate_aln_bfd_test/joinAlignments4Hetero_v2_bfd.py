        ...
        for line in f:
            if line.startswith(">"):
                ...
                if idx.startswith(name): 
                    ...
                    continue
                line=f.readline()
                #if idx in aln_dict.keys(): 
                #    present+=1
                #    pres.append(idx+"\n")
                aln_dict[idx+"@"+line.strip()]=line.strip()            
    """
    print("Count=",count)
    print("Present=",present)
    with open ("present_"+name+"_.txt","w") as f:
        f.writelines(pres)
    """
    return aln_dict
...
for idx_A in keys_A:
    ...
    for idx_B in keys_B:
        ...
        if aln_count>=35000: break
        #if aln_count>=35000: break
        if idx_A.startswith("tr|") and idx_B.startswith("tr|"):
            print (idx_A.split("|")[1],idx_B.split("|")[1])
            if "OS=" not in idx_A or "OS=" not in idx_B: continue
            if idx_A.split("OS=")[1].split()[0]+" "+idx_A.split("OS=")[1].split()[1] == idx_B.split("OS=")[1].split()[0]+" "+idx_B.split("OS=")[1].split()[1]:
                aln_count+=1
                pairs_AB.append([idx_A,idx_B])
            #pass
        elif idx_A.startswith("sp|") and idx_B.startswith("sp|"):
            if "OS=" not in idx_A or "OS=" not in idx_B: continue
            if idx_A.split("OS=")[1].split()[0]+" "+idx_A.split("OS=")[1].split()[1] == idx_B.split("OS=")[1].split()[0]+" "+idx_B.split("OS=")[1].split()[1]:
                aln_count+=1
                pairs_AB.append([idx_A,idx_B])
            #pass
        elif idx_A.startswith("ERR") and idx_B.startswith("ERR"):
            if idx_A.split()[0] == idx_B.split()[0]:
                aln_count+=1
                pairs_AB.append([idx_A,idx_B])
            #pass
        elif idx_A.startswith("SRR") and idx_B.startswith("SRR"):
            if idx_A.split()[0] == idx_B.split()[0]:
                aln_count+=1
                pairs_AB.append([idx_A,idx_B])
            #pass
        elif ("ID=" in idx_A) and ("ID=" in idx_B):
            if idx_A.split("ID=")[1].split(";")[0] == idx_B.split("ID=")[1].split(";")[0]:
                aln_count+=1
                pairs_AB.append([idx_A,idx_B])
            #pass
        elif idx_A.startswith("OM-") and idx_B.startswith("OM-"):
            if idx_A.split()[0] == idx_B.split()[0]:
                aln_count+=1
                pairs_AB.append([idx_A,idx_B])
            #pass
        elif idx_A.startswith("UPI") and idx_B.startswith("UPI"):
            if idx_A.split()[0] == idx_B.split()[0]:
                aln_count+=1
                pairs_AB.append([idx_A,idx_B])
            #pass
