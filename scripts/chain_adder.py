import copy
#usage: python chain_adder.py <input_pdb> <chain> <output_pdb>
...
def convert_to_correct(_pdb, _filename):
    # file is outputted
    array = []
    content = ''
    for x in _pdb:
        val = correct_format(x)
        array.append(val)
        content = content + val + '\n'
    f = open(_filename, "w")
    f.write(content + 'END')
...
import sys,os
input_model = os.path.abspath(sys.argv[1])#"/home/rajroy/CASP14/inputs_1/H1036_s2.pdb"
chain=sys.argv[2]
output_dir = os.path.abspath(sys.argv[3])#"/home/rajroy/H1036_s2.pdb"
model = contents_to_info(read_pdb(input_model))
model_a = add_chain(model, chain)
convert_to_correct(model_a, output_dir)
