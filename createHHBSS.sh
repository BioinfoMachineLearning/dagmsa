...
echo "Secondary structure information successfully added to $work_dir/$nameA.ss.a3m"

#For second MSA
perl $hhblits_dir/scripts/addss_v2020.pl $multicom_tool_dir $multicom_database_dir $work_dir/$nameB.a3m $work_dir/$nameB.ss.a3m -a3m
...
if [ -f $work_dir/$nameAB.hhr ]; then
    echo "Removing existing $work_dir/$nameAB.hhr ..."
...
if [ ! -f $work_dir/$nameAB.hhr ]; then
    echo "$work_dir/$nameAB.hhr is not generated by hhsuite search. Try one more time using $nameAB.ss.a3m ...\n"
    $hhsuite3_dir/bin/hhsearch -i $nameAB.ss.a3m -d $hhsuitedb -realign -mact 0
    if [ -f $work_dir/$nameAB.ss.hhr ]; then
        mv $nameAB.ss.hhr $nameAB.hhr
    fi
fi

if [ ! -f $work_dir/$nameAB.hhr ]; then
    echo "$work_dir/$nameAB.hhr is not generated by hhsuite search. Try one more time using $nameAB.a3m ...\n"
    $hhsuite3_dir/bin/hhsearch -i $nameAB.a3m -d $hhsuitedb -realign -mact 0
...
$meta_dir/script/rank_templates.pl $nameAB.hhr $work_dir/$nameAB.rank
